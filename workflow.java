...
public class workflow {
    ...
    /* Add node to LL. */
    void makeNewWorkflowItem(int application_ID) {
    ...
    int getNextWorkflowItem(app_status someStatus) {
        app_Node trav = head;
        
        while (trav != null) {
            if (trav.application_status == someStatus) { return trav.application_ID; }
            trav = trav.next;
        }
    ...
    void updateWorkflowItem(int app_ID, app_status someStatus) {
        app_Node trav = head;
        boolean found = false;
        
        /* Grabs the ID from the LL. */
        while (trav != null) {
            if (trav.application_ID == app_ID) { found = true; break; }
            trav = trav.next;
        }
